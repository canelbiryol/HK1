...
from partB.xMinuteReturn import getXMinuteTradeReturns
...
for x in range(len(plotWindows)):
    t_returns = getXMinuteTradeReturns(trades,plotWindows[x])
    plotAutocorrelation(t_returns, 20, plotWindows[x])
    
'''At this point, look at the data, the autocorrelation should drop off at around lag K=20?'''
K=20
confidence = 0.95
quantiles = np.zeros(10)
testWindows = np.array([5, 10, 20, 30, 60, 120, 300, 600, 900, 1800])
for x in range(len(testWindows)):
    t_returns = getXMinuteTradeReturns(trades,testWindows[x])
...
t_returns = getXMinuteTradeReturns(trades,optimal_Window)
q_returns = getXMinuteMidQuoteReturns(quotes,optimal_Window)

result = adfuller(t_returns, maxlag=None, regression='c', autolag='AIC', store=False, regresults=False)
...
result = adfuller(q_returns, maxlag=None, regression='c', autolag='AIC', store=False, regresults=False)
print('ADF Statistic: %f' % result[0])
print('p-value: %f' % result[1])
print('Critical Values:')
for key, value in result[4].items():
